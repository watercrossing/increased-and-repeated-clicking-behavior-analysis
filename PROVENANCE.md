...
How `study_data_merged.xlsx` was collected, by whom, where and when. Written for the
artifact evaluation; the authoritative account is the paper.
...
The submitted manuscript gives 86 for the last of these, which does not reconcile with its
own total (1625 − 138 − 419 − 86 = 982, not 986); 82 is the number the code produces and
the one the camera-ready text carries.
